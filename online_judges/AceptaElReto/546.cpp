...
int main(){
    ...
    cin >> tt;
    while(tt--) {
        set<ll> sa, sb, rea, reb;
        cin >> na;
        for(i = 0; i < na; i++) {
            cin >> num;
            if(sa.count(num)) rea.insert(num);
            else sa.insert(num);
        }
        cin >> nb;
        for(i = 0; i < nb; i++) {
            cin >> num;
            if(sb.count(num)) reb.insert(num);
            else sb.insert(num);
        }
        vll ansa, ansb;
        for(auto el : rea) {
            if(!sb.count(el)) ansa.pb(el);
        }
        for(auto el : reb) {
            if(!sa.count(el)) ansb.pb(el);
        }
        if(ansa.empty()) cout << "Nada que intercambiar\n";
        else cout << ansa << "\n";
        if(ansb.empty()) cout << "Nada que intercambiar\n";
        else cout << ansb << "\n";
    }
