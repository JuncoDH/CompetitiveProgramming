...
int main(){
    ...
    while(cin >> c >> f){
        ...
        int ansl = 0, ansr = 0;
        q.push(mp(0, mp(f/2, 0)));
        while(!q.empty()) {
            auto u = q.top(); q.pop();
            if(!is_valid(u.se.fi, u.se.se)) continue;
            if(visited[u.se.fi][u.se.se]) continue;
            visited[u.se.fi][u.se.se] = true;
            // echo(u, ansl, ansr);
            if(-u.fi <= ansl) {
                ansr++;
                for(auto el : dx)
                    if(is_valid(u.se.fi + el.fi, u.se.se + el.se))
                        q.push(mp(-v[u.se.fi + el.fi][u.se.se + el.se], mp(u.se.fi + el.fi, u.se.se + el.se)));
                // echo(q.size());
                continue;
            }
            if(2*ansr > f*c) break;
