...
const int MAX = 19;
ll ten[MAX]; // ten[i] = 10^i.
ll get_digit(ll num, int i) { // 0-indexed.
    return (num/ten[i])%10;
}
ll sum_digit(ll num) {
    ll res = 0, i;
    for(i = 0; i < MAX; i++) res += get_digit(num, i);
    return res;
}
void mod(int& rest) {
    rest = ((rest%3) + 3)%3;
}
ll count(ll num, int rest) { // Num of numbers <= num that are == rest mod 3.
    mod(rest);
    if(rest == 0) return num/3;
    if(rest == 1) return (num+2)/3;
    if(rest == 2) return (num+1)/3;
    return -1;
}


map<ll, ll> dp[3];
ll f(ll num, int rest) {
    mod(rest);
    if(num <= 0) return 0;
    if(dp[rest].count(num)) return dp[rest][num];
    ll i, g, s, ans = 0;
    for(i = 0; i < MAX; i++) {
        g = get_digit(num, i);
        s = sum_digit(num);
        if(g == 0) continue;
        if(g != s) {
            ans = f(num - g*ten[i], rest) + (s - g)*count(g*ten[i], rest - (s - g)) + f(g*ten[i], rest - (s - g));
        }
        else {
            if(g%3 == rest) ans = g;
            ans += f(num-1, rest);
    ...
    if(rand()%100 < 20) return dp[rest][num] = ans;
    return ans;
}
...
int main(){
    ...
    ll a, b, i;
    ten[0] = 1;
    for(i = 1; i < MAX; i++) ten[i] = ten[i-1]*10;
    while(cin >> a >> b) {
        if(!a && !b) return 0;
        cout << f(b, 0) - f(a-1, 0) << "\n";
    }
