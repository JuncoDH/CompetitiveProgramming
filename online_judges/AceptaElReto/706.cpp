...
vector<vector<pll>> graph, graph2;
...
int main(){
    ...
    while(cin >> n >> e) {
        ...
        dist_origin = dijkstra(0);
        dist_end = dijkstra(n-1);
        graph2.assign(n, {});
        for(i = 0; i < n; i++) {
            for(auto &el : graph[i]) {
                if(dist_end[i] < dist_end[el.fi]) continue;
                if(dist_end[i] == dist_end[0] - dist_origin[i] && dist_end[i] == dist_end[el.fi] + el.se) continue;
                graph2[el.fi].emplace_back(i, el.se);
                graph2[i].emplace_back(el.fi, el.se);
            }
        }
        swap(graph, graph2);
        dist_origin = dijkstra(0);
        if(dist_origin[n-1] == inf) cout << "IMPOSIBLE\n";
        else cout << dist_origin[n-1] << "\n";
    }
