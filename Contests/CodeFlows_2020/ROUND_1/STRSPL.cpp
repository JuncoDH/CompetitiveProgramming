...
int main(){
    ...
    while(tt--) {
        ...
        vll vi, vit, vj, vjt;
        for(i = 0; i <= n-i-1; i++) {
            if(i == n-i-1) vi.pb(i+1);
            else if(s[i] == s[n-i-1]) {vi.pb(i+1); vit.pb(n-i-1+1);}
            else {vj.pb(i+1); vjt.pb(n-i-1+1);}
        }
        cout << vi.size() + vit.size() << " " << vj.size() + vjt.size() << "\n";
        reverse(vit.begin(), vit.end());
        reverse(vjt.begin(), vjt.end());
        cout << vi << " " << vit << "\n";
        cout << vj << " " << vjt << "\n";
    }
