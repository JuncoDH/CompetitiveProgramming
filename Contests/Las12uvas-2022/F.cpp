...
vector<vll> elevate(vector<vll> mat, ll n) {
    if(n == 1) return mat;
    vector<vll> res = elevate(mat, n/2);
    vector<vll> aux = {{0, 0}, {0, 0}};
    for(int i = 0; i < 2; i++) {
        for(int j = 0; j < 2; j++) {
            for(int k = 0; k < 2; k++) {
                aux[i][j] += res[i][k]*res[k][j];
            }
        }
    }
    if(n&1) {
        vector<vll> aux2 = {{0, 0}, {0, 0}};
        for(int i = 0; i < 2; i++) {
            for(int j = 0; j < 2; j++) {
                for(int k = 0; k < 2; k++) {
                    aux2[i][j] += aux[i][k]*mat[k][j];
                }
            }
        }
        return aux2;
