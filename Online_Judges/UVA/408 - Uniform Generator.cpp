...
int main(){
    ...
    ll step, mod, original_step;
    bool ok;

    while(scanf("%lld %lld", &original_step, &mod) > 0) {
        step = original_step % mod;
        if(mod == 1) ok = true;
        else if(step == 0) ok = false;
        else ok = __gcd(step, mod) == 1;
        printf("%10lld%10lld    ", original_step, mod);
