...
ll dist(pii x, pii y) {
    return cuad(x.fi - y.fi) + cuad(x.se - y.se);
}
...
int main(){
    ...
    while(true) {
        ...
        for(i = 0; i < k; i++) {
            cin >> v[i].fi >> v[i].se;
            if(v[i].fi <= 10) dsu.Union(i, k);
            if(v[i].fi >= ancho-10) dsu.Union(i, k+1);
            for(j = 0; j < i; j++) {
                if(dist(v[i], v[j]) <= cuad(20)) dsu.Union(i, j);
            }
        }
        if(dsu.is_connected(k, k+1)) cout << "NO\n";
        else cout << "SI\n";
    }
