...
int main(){
    ...
    while(cin >> n >> m >> t) {
        ...
        ok = false;
        while(p.size() > 1 && -p.top().fi.fi <= t) {
            // echo(p.top(), t);
            auto d = p.top(); p.pop();
            auto e = p.top(); p.pop();
            // echo(d, e, colision(d.fi, e.fi));
            if(colision(d.fi, e.fi)) {ok = true; break;}
            p.push(e);
            if(d.se == 0) continue;
            d.fi.fi -= d.se;
            d.fi.se -= d.se;
            if(-d.fi.fi > t) continue;
            d.fi.se = -min(-d.fi.se, t);
            p.push(d);
        }
        if(ok) cout << "SI\n";
        else cout << "NO\n";
