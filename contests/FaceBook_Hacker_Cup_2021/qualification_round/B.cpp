...
int main(){
    ...
    for(zz = 1; zz <= tt; zz++) {
        ...
        for(i = 0; i < k; i++) {
            cin >> t;
            v[i] = mp((int)(t[0]-'A'), (int)(t[1]-'A'));
        }

        for(c = 0; c <= (int)('Z'-'A'); c++) {
            tans = 0;
            vll distance(27, inf);
            graph.assign(27, vector<pll>());
            for(auto el : v) graph[el.se].pb(mp(el.fi, 1));
            auto vans = dijkstra(c, 27);
            for(auto ch : s) {
                nch = ch-'A';
                if(vans[nch] == -1) {tans = inf; break;}
                tans += vans[nch];
            }
            ans = min(ans, tans);
        }
