...
int main(){
    ...
    for(zz = 1; zz <= tt; zz++) {
        ...
        for(i = k; i < r; i++) {
            ...
            for(j = 0; j < c; j++) sum[j] += v[i][j];
            ans = min(ans, abs(i-k) + s);
        }
        s = 0;
        for(j = 0; j < c; j++) if(sum[j] > k) s++;
        ans = min(ans, r-k + s);
        ...
        for(i = k; i >= 0; i--) {
            s = 0;
            for(j = 0; j < c; j++) {
                if(sum[j] > r-k-1 || v[i][j]) s++;
            }
            for(j = 0; j < c; j++) sum[j] += v[i][j];
            ans = min(ans, abs(i-k) + s);
        }
        s = 0;
        for(j = 0; j < c; j++) if(sum[j] > r-k-1) s++;
        ans = min(ans, abs(-1 -k) + s);
