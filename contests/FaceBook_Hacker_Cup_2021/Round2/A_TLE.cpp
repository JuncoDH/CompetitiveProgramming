...
int main(){
    ...
    ll tt, i, n, m, num, j, ans, zz;

    cin >> tt;
    for(zz = 1; zz <= tt; zz++) {
        cin >> n >> m;
        multiset<ll> used, nused, used_visited, nused_visited;
        for(i = 0; i < m; i++) {
            cin >> num;
            nused.insert(num);
        }
        ...
        for(i = 0; i < n; i++) {
            ...
            for(j = 0; j < m; j++) {
                ...
                if(used.count(num) > 0) {
                    ...
                    used_visited.insert(num);
                    continue;
                }
                if(nused.count(num) > 0) {
                    nused.erase(nused.find(num));
                    nused_visited.insert(num);
                    continue;
                }
                v.pb(num);
            }
            ...
            used.clear();
            nused.clear();
            for(auto el : used_visited) {
                used.insert(el);
            }
            for(auto el : nused_visited) {
                nused.insert(el);
            }
            for(auto el : v) {
                used.insert(el);
