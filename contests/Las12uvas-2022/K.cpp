...
int main(){
    ...
    ll n, k, i, l, r, mid, sum, ans, anspos;
    while(cin >> n >> k) {
        v = vi(n, 0);
        cin >> v;
        ...
        for(i = 0; i < n; i++) {
            if(i+k > n) break;
            sum = get_psum(i, i+k-1);
            l = i;
            r = i+k-1;
            while(l+1 < r) {
                mid = (l+r)/2;
                if(2*get_psum(i, mid) <= sum) l = mid;
                else r = mid;
            }
            if(2*get_psum(i, l) == sum) {
                if(sum > ans) {
                    ans = sum;
                    anspos = i;
                }
